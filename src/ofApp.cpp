...
// viewport
int vx = 43, vy = 290, vw = 967, vh = 726;
...
void ofApp::update(){
	...
	theta += 0.005;
	if (orbit) {
		cx = 450 + 100 * sin(theta);
		cy = 225 + 100 * cos(theta);
	}
	else {
		cx = 450;
		cy = 225;
...
void drawRect(int x1, int y1, int x2, int y2, ofColor fillColor) {
	...
	if (shatter == 1) {
		// break each rect into 8x particles
	...
	//hypnotize
	if (hypnotize) {
		for (int d = 2 + (ofGetFrameNum()/10) % 10; d < 40; d += 10) {
			ofSetColor(255 - d*6);
...
void ofApp::draw(){
	...
	if (shatter < 2) {
		int rIndex = 0;
		bool xShift = false;
		int xstep = square ? 50 : 100;
		int width = square ? 40 : 90;
		for (int y = 10; y < 400; y += 50) {
			for (int x = 10; x < 1000; x += xstep) {
				int xx = x + (xShift ? 50 : 0);
				drawRect(xx, y, xx + width, y + 45, palette[palIndex[rIndex]]);
				
				if (showID) {
					ofSetColor(0, 255, 0);
					font.drawString(to_string(rIndex), xx + 50, y + 35);
				}
				rIndex++;
			}
			if (!square) xShift = !xShift;
		}
		if (shatter == 1) {
			nextPart = 0;
			shatter = 2;
...
*/
	...
	case OF_KEY_RIGHT:
		if ((GetKeyState(VK_SHIFT)&0x80) == 0x80) {
			switch (key) {
			case OF_KEY_UP:	vh--; break;
			case OF_KEY_DOWN: vh++; break;
			case OF_KEY_LEFT: vw--; break;
			case OF_KEY_RIGHT: vw++; break;
			}
		}
		else {
			switch (key) {
			case OF_KEY_UP:	vy--; break;
			case OF_KEY_DOWN: vy++; break;
			case OF_KEY_LEFT: vx--; break;
			case OF_KEY_RIGHT: vx++; break;
			}
		}
